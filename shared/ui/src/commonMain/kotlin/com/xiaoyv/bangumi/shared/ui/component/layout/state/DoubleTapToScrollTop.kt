...
internal class DoubleTapToScrollTopState {
    ...
    val isScrolled: Boolean get() = delegatedState?.isScrolled ?: ownIsScrolled
    val showHint: Boolean get() = delegatedState?.showHint ?: ownShowHint
    private var scrollToTop: (() -> Unit)? = null

    fun update(isScrolled: Boolean, scrollToTop: () -> Unit) {
    ...
    fun delegateTo(state: DoubleTapToScrollTopState) { if (state !== this) delegatedState = state }
    fun clearDelegate(state: DoubleTapToScrollTopState) { if (delegatedState === state) delegatedState = null }
    fun invokeScrollToTop() { delegatedState?.invokeScrollToTop() ?: scrollToTop?.invoke() }
    fun dismissHint() { delegatedState?.dismissHint() ?: run { ownShowHint = false } }
}

internal val LocalDoubleTapToScrollTopState = staticCompositionLocalOf<DoubleTapToScrollTopState?> { null }
...
internal fun DoubleTapToScrollTopHost(
    ...
    Layout(
        content = {
            ...
            Box(modifier = Modifier.fillMaxWidth().height(DoubleTapTopAreaHeight)) {
                DoubleTapToScrollTopOverlay(
                    state,
                    Modifier.fillMaxSize().padding(horizontal = DoubleTapTopAreaHorizontalPadding).zIndex(1f),
                )
            }
        },
...
internal fun BoxScope.DoubleTapToScrollTop(
    ...
    SideEffect { state.update(isScrolled) { scope.launch { currentOnScrollToTop() } } }
...
@Composable
private fun DoubleTapToScrollTopOverlay(state: DoubleTapToScrollTopState, modifier: Modifier) {
    ...
    if (state.isScrolled) {
        Box(modifier = modifier.pointerInput(state) { detectTapGestures(onDoubleTap = { state.invokeScrollToTop() }) }) {
            if (state.showHint) {
                Text(
